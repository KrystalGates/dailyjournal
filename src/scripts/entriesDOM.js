...
function recordBtnListener(){
  ...
  let newEntry = buildJournalEntry(journalDate, conceptsCovered, journalEntry, moodForDay)
...
function formValidationCharacters(){
  ...
  if(conceptsCovered.value.match(allowedChar) && journalEntry.value.match(allowedChar)){
    return true
  } else {
...
function buildJournalEntry(journalDate, conceptsCovered, journalEntry, moodForDay){
  return ({
